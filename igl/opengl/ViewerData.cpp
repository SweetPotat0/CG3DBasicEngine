...
IGL_INLINE void igl::opengl::ViewerData::normal_matcap()
{
    ...
    static const auto clamp = [](double t)
                ...
                for (unsigned i = 0; i < this->F.rows(); ++i)
                    for (unsigned j = 0; j < 3; ++j)
                        meshgl.V_uv_vbo.row(i * 3 + j) =
                            this->V_uv.row(per_corner_uv ? this->F_uv(i, j) : this->F(i, j)).cast<float>();
            }
        }
    }
    else
            ...
            for (unsigned i = 0; i < this->F.rows(); ++i)
                for (unsigned j = 0; j < 3; ++j)
                    meshgl.V_normals_vbo.row(i * 3 + j) =
                        per_corner_normals ? this->F_normals.row(i * 3 + j).cast<float>() : this->F_normals.row(i).cast<float>();
