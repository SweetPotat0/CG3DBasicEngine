...
void handlePicking(double xStart, double yStart, double xEnd, double yEnd, Project *scn, Renderer *rndr)
{
    ...
    if (scn->zoomInMode == true)
    {
        float n = (rndr->GetFar(0) - rndr->GetNear(0)) / 5;
        if (abs(xStart - xEnd) > abs(yStart - yEnd))
        {
            float dr = abs(xStart - xEnd);
            float x = n * dr * 0.5;
            std::cout << "dr: " << dr << std::endl;
            std::cout << "n: " << n << std::endl;
            std::cout << "0.5 * dr: " << 0.5 * dr << std::endl;
            std::cout << "x: " << x << std::endl;
            rndr->MoveCamera(0, rndr->zTranslate, x - n);
        }
        else
        {
            float dr = abs(yStart - yEnd);
            float x = n * dr * 0.5;
            std::cout << "dr: " << dr << std::endl;
            std::cout << "n: " << n << std::endl;
            std::cout << "0.5 * dr: " << 0.5 * dr << std::endl;
        ...
        scn->zoomInMode = false;
        std::cout << "near: " << rndr->GetNear(0) << std::endl;
    }
    else
    {
